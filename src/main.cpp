...
/*int main() {
        ...
        vector<string> names = { "game1_clip1", "game1_clip2", "game1_clip3",
   "game1_clip4", "game2_clip1", "game2_clip2", "game3_clip1", "game3_clip2",
                                                         "game4_clip1",
   "game4_clip2" };
        ...
        for (const string& name : names) {
                ...
                if (image.empty()) {
                        cerr << "Error loading image file: " << imagePath <<
   endl; return -1;
                }
        ...
        vector<vector<vector<int>>> allLabels; // Vector to store the labels
   from all label files vector<vector<int>> gtClassId(names.size()); double
   overallAP = 0.0; vector<double> overallAPPerClass(6, 0.0); vector<double>
   classAPSum(6, 0.0); // Accumulate AP for each class vector<int> classCount(6,
   0); // Count number of instances per class

        // Load images and extract labels from label files
        for (int i = 0; i < names.size(); i++) {
                const string& name = names[i];
                string labelPath = base_path + name +
   "/bounding_boxes/frame_first_bbox.txt";
                ...
                if (extractLabelsFromFile(labelPath, labels)) {
                        ...
                        for (int j = 0; j < labels.size(); j++) {
                                ...
                                cout << "  First int: " << lineLabels[0]
                                        ...
                                        << ", Fifth int: " << lineLabels[4] <<
   endl; gtClassId[i][j] = lineLabels[4];
                        }
                        allLabels.push_back(labels); // Store labels in the
   vector
                }
                else {
                        cerr << "Failed to find all required labels in file: "
   << labelPath << endl;
                }
        }
   ...
   from the rest of the image Vec4Points vertices = detect_field(in_img);
                ...
                line(cutout_table, vertices[0], vertices[1], linecolor,
   linewidth); line(cutout_table, vertices[2], vertices[1], linecolor,
   linewidth); line(cutout_table, vertices[2], vertices[3], linecolor,
   linewidth); line(cutout_table, vertices[3], vertices[0], linecolor,
   linewidth); imshow("out", cutout_table); Mat cutout_original =
   cutout_table.clone(); imshow("mask", mask);
                ...
                for (int i = 0; i < detected_balls.size(); ++i) {
                        Point2f ball = Point2f(detected_balls[i][0],
   detected_balls[i][1]); float radius = detected_balls[i][2]; if
   (!(is_ball_near_line(ball, radius, vertices[0], vertices[1]) ||
                                is_ball_near_line(ball, radius, vertices[1],
   vertices[2]) || is_ball_near_line(ball, radius, vertices[2], vertices[3]) ||
                                is_ball_near_line(ball, radius, vertices[3],
   vertices[0]))) { selected_balls.push_back(detected_balls[i]);
                        }
                }

                // Draw detected balls and bounding boxes
                vector<vector<cv::Point2f>> vertices_boxes =
   compute_bbox_vertices(selected_balls); Mat classifiedImg = in_img.clone();
                vector<int> predClassId(vertices_boxes.size());
                vector<cv::Rect> bbox_rectangles =
   compute_bboxes(selected_balls);
                ...
                for (int j = 0; j < vertices_boxes.size(); j++) {
                        ...
                        Rect rect(box[0] - Point2f(5, 5),
                                box[2] + Point2f(5, 5)); // Assuming box[0] is
   top-left and box[2] is bottom-right Mat roi = in_img(rect); Mat ballroi =
   detectBalls(roi);
                        ...
                        if (classifiedBall == ball_class::STRIPED) {
                                rectangle(classifiedImg, rect, Scalar(0, 255,
   0), 2); stripped_balls.push_back(center); circle(mask_stripped, center,
   radius, Scalar(255), -1);
                        }
                        else if (classifiedBall == ball_class::SOLID) {
                                rectangle(classifiedImg, rect, Scalar(0, 0,
   255), 2); solid_balls.push_back(center); circle(mask_solid, center, radius,
   Scalar(255), -1);
                        }
                        else if (classifiedBall == ball_class::CUE) {
                                rectangle(classifiedImg, rect, Scalar(255, 255,
   255), 2); // White for white ball white_balls.push_back(center);
                                circle(mask_white, center, radius, Scalar(255),
   -1);
                        }
                        else if (classifiedBall == ball_class::EIGHT_BALL) {
                                rectangle(classifiedImg, rect, Scalar(0, 0, 0),
   2); // Black for black ball black_balls.push_back(center); circle(mask_black,
   center, radius, Scalar(255), -1);
                        }
                        predClassId[j] = classId;
                        cout << "Class ID: " << classId << endl; // Print the
   class ID
                }
                ...
                Mat ballMasks[4] = { mask_stripped, mask_solid, mask_white,
   mask_black }; int values[4] = { 1, 2, 3, 4 };

                for (int k = 0; k < 4; ++k) {
                        Mat ballMask = ballMasks[k];
                        Mat resizedBallMask;
                        resize(ballMask, resizedBallMask, combinedMask.size(),
   0, 0, INTER_NEAREST); combinedMask.setTo(Scalar(values[k]), resizedBallMask);
                }

                // Compute IoU for each class
                vector<double> classIoUs(6, 0.0);
                for (int c = 0; c < 6; ++c) {
                        classIoUs[c] = ComputeIoUPerClass(combinedMask,
   gtMasks[i], c);
                }
...
// Print results
for (int c = 0; c < 4; ++c) {
        cout << "Class " << c + 1 << " Average Precision: " << meanAPPerClass[c]
<< endl;
}
...
int main(int argc, char **argv) {
  ...
  fillPoly(fieldMask, vertices, Scalar(255));
  combinedMask.setTo(Scalar(5), fieldMask);
  imshow("combinedMask", combinedMask);
