...
int main(int argc, char **argv) {
  ...
  if (parser.has("benchmark")) {
    ...
    double averagePrecision = computeAP(gtBoxes, predBoxes, gt_classes,
                                        pred_classes, ball_class::SOLID);
    waitKey();
  }
  return 0;
}
...
vector<string> names = {
    "game1_clip1", "game1_clip2", "game1_clip3", "game1_clip4", "game2_clip1",
    "game2_clip2", "game3_clip1", "game3_clip2", "game4_clip1", "game4_clip2"};
...
for (int i = 0; i < images.size(); i++) {
  ...
  double averagePrecision =
      computeAveragePrecision(gtBoxes, predBoxes, gtClassId[i], predClassId, 4);

  waitKey(0);
/*
 vector<double> ap_per_class = computeAveragePrecisionPerClass(
     gtBoxes, predBoxes, gtClassId[i], predClassId, 4);
 ...
 cout << "Mean Average Precision (mAP) for image " << names[i] << ": " <<
 mAP
      << endl;
...
// Visualize ground truth and predicted boxes (visualization code omitted
// for brevity)
